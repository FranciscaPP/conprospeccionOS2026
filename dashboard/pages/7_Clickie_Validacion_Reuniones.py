"""Portal cliente Clickie — Validación de Reuniones."""
...
st.set_page_config(page_title="Clickie — Validación Reuniones", layout="wide", page_icon="")

SUPABASE_URL = supabase_url()
SUPABASE_KEY = supabase_key()
GHL_TOKEN = ghl_tokens().get("clickie", "")
...
def cargar_reuniones(fi, ff):
    url = (f"{SUPABASE_URL}/rest/v1/vw_reuniones_semana"
           f"?select=*&cliente_slug=eq.clickie&fecha=gte.{fi}&fecha=lte.{ff}&order=fecha.asc,hora.asc")
...
def cargar_stages():
    r = requests.get(
        ...
        f"?select=pipeline_id,pipeline_name,stage_id,stage_category&cliente_slug=eq.clickie",
...
def cargar_seguimiento():
    r = requests.get(
        f"{SUPABASE_URL}/rest/v1/clickie_seguimiento?select=reunion_id,status_comercial,etapa_comercial",
        headers=_H, timeout=15)
    if not r.ok: return {}
...
def upd_seguimiento(rid, status, etapa):
    return requests.post(
        f"{SUPABASE_URL}/rest/v1/clickie_seguimiento",
...
def render_buscador():
    st.markdown(
        ...
        '<div style="font-size:15px;font-weight:700;color:#4c1d95;margin-bottom:8px">'
        'Verificar si un contacto ya tuvo reuniones con Clickie</div>'
        '<div style="font-size:12px;color:#6d28d9;margin-bottom:10px">'
    ...
    with col_inp:
        term = st.text_input(
            "Buscar contacto",
            placeholder="ej: rodrigo@empresa.cl / Empresa SA / +56912345678",
            label_visibility="collapsed",
            key="ck_buscador_term",
        )
    with col_btn:
        buscar = st.button("Buscar", key="ck_buscador_btn", use_container_width=True)
    ...
    if buscar or term:
        ...
        if df_res.empty:
            st.markdown(
                ...
                'Este contacto no ha tenido reuniones previas con Clickie.</div>',
                unsafe_allow_html=True)
        else:
            n = len(df_res)
            st.markdown(
                ...
                f'Encontramos <b>{n}</b> reunión{"es" if n > 1 else ""} previas con Clickie</div>',
                unsafe_allow_html=True)
...
def render_header():
    ...
    st.markdown(
        ...
        f'border-radius:14px;border:1px solid #ddd6fe;margin-bottom:20px;'
...
def run():
    ...
    with c_mes:
        mes = st.selectbox("Mes", range(1,13),
                           ...
                           index=hoy.month-1, key="ck_mes")
    with c_anio:
        anio = int(st.number_input("Año", 2024, 2030, hoy.year, key="ck_anio"))
    with c_ref:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("Actualizar", key="ck_ref"):
    ...
    if not df.empty:
        ...
        with c_dia:
            dia_sel = st.selectbox("Día", opts_dia, key="ck_dia")
        with c_estado_f:
            estado_f = st.selectbox("Estado de validación", _FILTRO_ESTADO_OPTS, key="ck_estado_f")
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        with st.container():
            st.markdown(
                f'<div style="background:#f5f3ff;padding:10px 16px;border-radius:10px 10px 0 0;'
                f'border:1px solid #ddd6fe;display:flex;align-items:center;gap:10px;flex-wrap:wrap">'
                f'<span style="font-size:13px;font-weight:700;color:#334155">{fecha_txt}</span>'
                f'<span style="color:#94a3b8">·</span>'
                f'<span style="font-size:15px;font-weight:800;color:#6d28d9">{hora}</span>'
                + badge_html + '</div>',
                unsafe_allow_html=True)
            ...
            with c3:
                nuevo_status = st.text_area("Status comercial",
                    ...
                    key=f"ck_status_{rid}_{i}")

            _, col_btn = st.columns([5, 1])
            with col_btn:
                if st.button("Guardar", key=f"ck_save_{rid}_{i}", type="primary"):
