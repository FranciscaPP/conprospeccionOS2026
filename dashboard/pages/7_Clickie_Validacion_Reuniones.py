"""Portal cliente Clickie - validacion contractual de reuniones."""
...
from shared.clickie_brand import GBS_BORDER_2, GBS_DARK, GBS_PURPLE, GBS_PURPLE_BG, ICP_DEFAULT
...
st.set_page_config(
    page_title="Clickie - Validacion Reuniones",
    layout="wide",
    page_icon="",
)
...
def cargar_reuniones(fecha_inicio, fecha_fin):
    ...
    url = (
        ...
        f"&cliente_slug=eq.clickie&excluida=eq.false"
...
def cargar_seguimiento():
    base_url = (
        ...
        f"?select={COLUMNAS_CLIENTE}&cliente_slug=eq.clickie"
    )
...
def buscar_contacto(term):
    ...
    url = (
        f"{SUPABASE_URL}/rest/v1/reuniones?select={fields}&cliente_slug=eq.clickie"
        f"&excluida=eq.false"
...
def render_header():
    goal = meta_de("clickie") or {"validas": 0}
    ...
    st.markdown(
        ...
        f'{img_b64("clickie_logo.png", 42)}{img_b64("conprospeccion_logo.png", 36)}</div></div>',
...
def render_buscador():
    term = st.text_input(
        ...
        key="clickie_busqueda_historica",
    )
...
def sincronizar_busqueda_principal():
    ...
    if not _clean(st.session_state.get("clickie_search")):
...
def cargar_icp_resumen():
    ...
    try:
        response = requests.get(
            ...
            f"?select={fields}&cliente=eq.clickie&limit=1",
            headers=_H,
            timeout=10,
        )
...
def set_kpi_filter(value):
    st.session_state["clickie_kpi_filter"] = value
...
def render_kpis(base_df):
    ...
    active = st.session_state.get("clickie_kpi_filter")
    palette = {
    ...
    for key, (background, border, color) in palette.items():
        ...
        styles.append(
            f'div[class*="st-key-clickie_kpi_{key}"] button{{'
            ...
            f'div[class*="st-key-clickie_kpi_{key}"] button p{{'
...
def aplicar_kpi(df):
    active = st.session_state.get("clickie_kpi_filter", "all")
    if active == "valid":
        return df[
...
def tabla_reuniones(df):
    ...
    descending = st.session_state.get("clickie_fecha_desc", True)
    ordered = df.copy()
    ...
    with st.container(key="clickie_meeting_table_header"):
        ...
        if header[0].button(
            "↓" if descending else "↑",
            key="clickie_sort_fecha",
            help="Ordenar por fecha",
        ):
            st.session_state["clickie_fecha_desc"] = not descending
            st.rerun()
    ...
    if True:
        for _, row in ordered.iterrows():
            ...
            if columns[6].button(
                "Ver detalle",
                key=f"clickie_detalle_{int(row['id'])}",
                use_container_width=True,
            ):
                abrir_detalle(row)
            st.markdown(
...
def render_evidencia(row, seg, *, cotizacion=False):
    if cotizacion:
        st.markdown(
            ...
            'demuestra interés inmediato por el servicio de Clickie y se realiza '
...
def render_detalle(row):
    ...
    if st.button(
        "Ver ICP acordado →",
        key=f"clickie_icp_link_{reunion_id}",
        help="Abrir el resumen ICP confirmado en Onboarding",
    ):
        st.session_state["clickie_scroll_to_icp"] = True
        st.switch_page("pages/17_Clickie_Onboarding.py")
    if not is_quote:
    ...
    if not is_quote:
        st.markdown(
            ...
            'padding:9px 12px;margin-top:9px;font-size:11px;color:#6d28d9;line-height:1.45">'
    ...
    if is_quote:
        st.info(
            "Cotización válida automáticamente por interés inmediato y traspaso al equipo comercial de Clickie."
        )
    elif future:
...
def run():
    ...
    render_client_nav("7_Clickie_Validacion", "clickie")
    st.markdown(
        f"""
        <style>
        button[kind="primary"] {{
        ...
        div[class*="st-key-clickie_icp_link_"] button {{
        ...
        div[class*="st-key-clickie_icp_link_"] button p {{
            color:{GBS_PURPLE}!important;
        ...
        div[data-testid="stLayoutWrapper"]:has(> .st-key-clickie_meeting_table_header) {{
            position:sticky!important;
            top:2.75rem!important;
            z-index:99999!important;
            background:linear-gradient(90deg,#1e293b,#7c3aed)!important;
            padding:9px 12px!important;
            border-radius:10px!important;
            box-shadow:0 4px 10px rgba(15,23,42,.10)!important;
            width:100%!important;
        }}
        .st-key-clickie_meeting_table_header {{
            background:transparent!important;
        }}
        .st-key-clickie_meeting_table_header button {{
            background:#fff!important;
            min-height:36px!important;
        }}
        div[data-testid="stVerticalBlock"]:has(.st-key-clickie_meeting_table_header),
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.st-key-clickie_meeting_table_header) {{
