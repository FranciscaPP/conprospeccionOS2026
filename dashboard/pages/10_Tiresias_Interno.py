"""
Dashboard interno — Reuniones Tiresias.
Vista para el equipo Conprospección: incluye SDR, pipeline info y herramientas técnicas.
"""
import sys, base64, calendar, requests, pandas as pd, streamlit as st
...
st.set_page_config(page_title="Tiresias — Reuniones", layout="wide", page_icon="")
...
OPCIONES_VAL = {
    "Pendiente de validación": "pendiente_validacion",
...
_VAL_LABEL.update({
    ...
    "reunion_agendada": "Pendiente de validación",
...
ESTADO_STYLE = {
    ...
    "reunion_agendada": ("#e0f2fe","#075985","Agendada"),
...
def mover_ghl(opp_id, pid, sid):
    if not all([opp_id, pid, sid, GHL_TOKEN]): return "sin datos GHL"
    ...
    return "GHL " if r.ok else f"GHL {r.status_code}"
...
def guardar_reunion(rid, opp_id, nuevo_estado, estado_ant, nueva_etapa, etapa_ant,
    ...
    if nuevo_estado != estado_ant:
        if upd_validacion(rid, nuevo_estado):
            msgs.append(f"validación {_VAL_LABEL.get(nuevo_estado, nuevo_estado)}")
            cat = VAL_A_CAT.get(nuevo_estado)
            if cat and cat in stages:
                msgs.append(mover_ghl(opp_id, *stages[cat]))
        else:
            msgs.append("error validación")

    seg_cambio = nueva_etapa != etapa_ant or nuevo_status.strip() != status_ant.strip()
    if seg_cambio:
        if upd_seguimiento(rid, nuevo_status, nueva_etapa):
            msgs.append("notas ")
...
def buscar_contacto(term: str) -> pd.DataFrame:
    ...
    url = (
        ...
        f"?select=contacto,empresa,email,telefono,cargo,industria,fecha_reunion,hora_reunion,estado_validacion,sdr_slug"
...
def render_buscador():
    st.markdown(
        ...
        'Busca por correo, nombre de empresa, teléfono o nombre del contacto (historial completo)</div>'
        '</div>',
    ...
    if buscar or term:
        ...
        if df_res.empty:
            ...
            st.markdown(
                ...
                f'padding:14px 18px;color:#1e3a8a;font-weight:700;margin-bottom:10px">'
            ...
            for _, row in df_res.iterrows():
                fecha_raw = row.get("fecha_reunion") or ""
                try:
                ...
                hora_str = str(row.get("hora_reunion") or "")[:5] or "—"
                contacto = _safe(row.get("contacto"), "—").title()
                empresa = _safe(row.get("empresa"), "—").title()
                cargo = _safe(row.get("cargo"), "")
                email = _safe(row.get("email"), "—")
                tel = _safe(row.get("telefono"), "—")
                sdr_slug = _safe(row.get("sdr_slug"), "")
                estado = _safe(row.get("estado_validacion"), "pendiente_validacion")
                s_bg, s_color, s_lbl = ESTADO_COLOR.get(estado, ("#f3f4f6","#374151","—"))

                sdr_badge = (f'<span style="background:#dbeafe;color:#1e40af;padding:1px 8px;'
                             f'border-radius:8px;font-size:11px;font-weight:600;margin-left:6px">'
                             f'{sdr_slug.replace("_"," ").title()}</span>') if sdr_slug else ""
                ...
                st.markdown(
                    ...
                    f'border-radius:8px;font-size:11px;font-weight:600">{s_lbl}</span>'
                    + sdr_badge + '</div>'
...
def render_header():
    ...
    st.markdown(
        ...
        f'<div><div style="font-size:22px;font-weight:800;color:#1e293b">Dashboard de Reuniones</div>'
        f'<div style="font-size:13px;color:#64748b;margin-top:3px">Vista interna · '
        f'Pipeline <b style="color:#1e40af">Clickie Sales</b></div></div></div>{c}</div>',
        unsafe_allow_html=True)
...
def run():
    ...
    with c_mes:
        mes = st.selectbox("Mes", range(1,13),
                           ...
                           index=hoy.month-1, key="t_mes")
    with c_anio:
        anio = int(st.number_input("Año", 2024, 2030, hoy.year, key="t_anio"))
    with c_ref:
        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("Actualizar", key="t_ref"):
    ...
    if not df.empty:
        ...
        opts_dia = ["Todos los días"] + [
            ...
            for f in fechas]
        dia_sel = st.selectbox("Día", opts_dia, key="t_dia")
        if dia_sel != "Todos los días":
            idx = opts_dia.index(dia_sel) - 1
            df = df[df["fecha"] == fechas[idx]].copy()

    with st.expander("SQL — crear tabla tiresias_seguimiento (solo primera vez)", expanded=False):
        st.code("""CREATE TABLE IF NOT EXISTS tiresias_seguimiento (
    reunion_id BIGINT PRIMARY KEY, status_comercial TEXT,
    etapa_comercial TEXT, updated_at TIMESTAMPTZ NOT NULL DEFAULT now());""", language="sql")

    if df.empty:
        st.info(f"No hay reuniones para el filtro seleccionado.")
        return
    ...
    n_pend = total - n_val - n_nv

    m1,m2,m3,m4 = st.columns(4)
    m1.metric("Total", total); m2.metric("Válidas", n_val)
    m3.metric("No válidas", n_nv); m4.metric("Pendientes", n_pend)
    ...
    st.markdown(
        ...
        f'Reuniones — {MESES_ES[int(mes)-1].capitalize()} {anio} '
        f'<span style="font-weight:400;font-size:12px;opacity:.85">(más antigua más nueva)</span></div>',
        unsafe_allow_html=True)
    st.markdown("---")
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        sdr = _safe(row.get("sdr") or row.get("sdr_slug"), "Sin asignar").title()
        ...
        s_bg, s_color, s_lbl = ESTADO_STYLE.get(estado_db, ("#f3f4f6","#374151","—"))
        lbl_val = _VAL_LABEL.get(estado_db, _VAL_LABEL.get(
            estado_db.replace("reunion_",""), "Pendiente de validación"))
        ...
        with st.container():
            st.markdown(
                ...
                f'<span style="color:#94a3b8">·</span>'
                + badge(sdr, "#dbeafe", "#1e40af")
                + badge(s_lbl, s_bg, s_color) + '</div>',
                unsafe_allow_html=True)

            col_c, col_e = st.columns(2)
            with col_c:
                pais_html = f'<div style="font-size:12px;color:#64748b">{pais}</div>' if pais else ""
                st.markdown(
            ...
            with c1:
                nuevo_estado = st.selectbox("Validación",
                    list(OPCIONES_VAL.keys()),
                    index=list(OPCIONES_VAL.keys()).index(lbl_val) if lbl_val in OPCIONES_VAL else 0,
                    key=f"val_{rid}_{i}")
                nuevo_estado = OPCIONES_VAL[nuevo_estado]
            with c2:
                ...
                nueva_etapa_lbl = st.selectbox("Etapa comercial", etapa_keys,
                    ...
                    key=f"etapa_{rid}_{i}")
                nueva_etapa = ETAPAS[nueva_etapa_lbl]
            with c3:
                nuevo_status = st.text_area("Status comercial",
                    value=status_prev, height=88,
                    placeholder="Ej: Propuesta enviada el 26 mayo, seguimiento el 2 junio...",
                    key=f"status_{rid}_{i}")

            _, col_btn = st.columns([5, 1])
            with col_btn:
                if st.button("Guardar", key=f"save_{rid}_{i}", type="primary"):
                    msgs = guardar_reunion(rid, opp_id, nuevo_estado, estado_db,
