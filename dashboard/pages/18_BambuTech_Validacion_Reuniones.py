"""Portal cliente BambuTech Services - validacion contractual de reuniones."""
...
st.set_page_config(
    page_title="BambuTech Services - Validacion Reuniones",
    layout="wide",
    page_icon="",
)
...
PROSPECCION_INICIO = date(2026, 5, 18)
...
VALIDATION_STYLE = {
    ...
    "pendiente_confirmacion_cliente": ("#ecf9ec", "#208d25"),
...
def cargar_reuniones(fecha_inicio, fecha_fin):
    base_fields = (
        ...
        "ghl_contact_id,opportunity_id,origen_reunion,excluida,titulo"
    )
    fields = f"{base_fields},informacion_reunion,bant_sdr"
    url = (
        f"{SUPABASE_URL}/rest/v1/reuniones?select={fields}"
        f"&cliente_slug=eq.bambutech&excluida=eq.false"
    ...
    if not df.empty:
        ...
        contacts_with_calendar = set(
            df.loc[
                df["origen_reunion"].isin(["calendar_appointment", "ghl_calendar"]),
                "ghl_contact_id",
            ].dropna()
        )
        df = df[
...
def cargar_seguimiento():
    base_url = (
        ...
        f"?select={COLUMNAS_CLIENTE}&cliente_slug=eq.bambutech"
    )
...
def buscar_contacto(term):
    ...
    url = (
        f"{SUPABASE_URL}/rest/v1/reuniones?select={fields}&cliente_slug=eq.bambutech"
        f"&excluida=eq.false"
...
def enriquecer(df, seguimiento):
    ...
    for _, source in df.iterrows():
        ...
        if validation_source in {"reagendar", "reagendada"}:
            source_status = "pendiente_reagendar"
        elif validation_source in {"reunion_valida", "válida", "valida"}:
            source_status = "realizada"
        status = (
            "cotizacion"
...
def render_buscador():
    term = st.text_input(
        ...
        key="bambutech_busqueda_historica",
    )
...
def sincronizar_busqueda_principal():
    ...
    if not _clean(st.session_state.get("bambutech_search")):
        st.session_state["bambutech_kpi_filter"] = None
...
def cargar_icp_resumen():
    ...
    try:
        response = requests.get(
            ...
            f"?select={fields}&cliente=eq.bambutech&limit=1",
            headers=_H,
            timeout=10,
        )
...
def set_kpi_filter(value):
    st.session_state["bambutech_kpi_filter"] = value
...
def render_kpis(base_df):
    ...
    active = st.session_state.get("bambutech_kpi_filter")
    palette = {
        "all": ("#eff6ff", "#bfdbfe", "#1d4ed8"),
        "valid": ("#f0fdf4", "#86efac", "#166534"),
        "not_valid": ("#fef2f2", "#fecaca", "#991b1b"),
        "pending_client": ("#ecf9ec", "#b8dfba", "#208d25"),
    ...
    for key, (background, border, color) in palette.items():
        ...
        styles.append(
            f'div[class*="st-key-bambutech_kpi_{key}"] button{{'
            ...
            f'div[class*="st-key-bambutech_kpi_{key}"] button p{{'
    ...
    st.markdown(
        ...
        f'<span style="font-size:13px;font-weight:800;color:#208d25;white-space:nowrap">'
        f'Avance de meta · {validas}/{target} ({pct}%)</span>'
        f'<div style="height:6px;flex:1;background:#ecf9ec;border-radius:6px;overflow:hidden">'
...
def aplicar_kpi(df):
    active = st.session_state.get("bambutech_kpi_filter", "all")
    if active == "valid":
        return df[
...
def tabla_reuniones(df):
    ...
    descending = st.session_state.get("bambutech_fecha_desc", True)
    ordered = df.copy()
    ordered["_fecha_orden"] = pd.to_datetime(
        ordered["fecha"].where(ordered["fecha"].notna(), ordered["fecha_reunion"]),
        errors="coerce",
    )
    ...
    widths = [0.85, 1.6, 1.55, 1.35, 1.45, 1.9, 1]
    ...
    with st.container(key="bambutech_meeting_table_header"):
        ...
        if header[0].button(
            "↓" if descending else "↑",
            key="bambutech_sort_fecha",
            help="Ordenar por fecha",
        ):
            st.session_state["bambutech_fecha_desc"] = not descending
            st.rerun()
    ...
    if True:
        for _, row in ordered.iterrows():
            ...
            if date_value is None or pd.isna(date_value):
                date_value = row.get("fecha_reunion")
            ...
            agenda_stage = row.get("_agenda_stage")
            validation_status = row.get("_validation_status")

            columns = st.columns(widths, vertical_alignment="center")
            columns[0].markdown(
                f'<div style="font-size:13px;font-weight:700;color:#0f172a">{short_date}</div>'
                f'<div style="font-size:12px;color:#64748b">{_safe_html(hour)}</div>',
                unsafe_allow_html=True,
            )
            columns[1].markdown(
            ...
            agenda_html = (
                ...
                f'padding:5px 10px;border-radius:9px;font-size:12px;font-weight:750;'
            ...
            if columns[6].button(
                "Ver detalle",
                key=f"bambutech_detalle_{int(row['id'])}",
                use_container_width=True,
            ):
                abrir_detalle(row)
            st.markdown(
...
def render_evidencia(row, seg, *, cotizacion=False):
    if cotizacion:
        st.markdown(
            '<div style="background:#f4f6f4;border:1px solid #d8b4fe;'
            ...
            'demuestra interés inmediato por el servicio de BambuTech Services y se realiza '
    ...
    if confirmation:
        st.markdown(
            ...
            'align-items:start;background:#f4f6f4;border:1px solid #d8b4fe;'
            'border-left:5px solid #38d430;border-radius:10px;padding:12px 14px;'
            'margin-top:8px">'
        ...
        if int(row.get("id") or 0) == 5089:
            st.image(
                str(DASHBOARD_DIR / "assets" / "bambutech_contract_clause_validity.png"),
                caption="Respaldo contractual · cláusula Tercera, numeral 4",
                use_container_width=True,
            )
    else:
        st.markdown(
...
def render_historial(reunion_id, row):
    ...
    if not is_quote and row.get("_agenda_stage") != "reagendar" and (row.get("_status") == "realizada" or (
    ...
    validation_at = (
        row.get("_seg", {}).get("validated_final_at")
        or row.get("_seg", {}).get("validated_cp_at")
        or row.get("_seg", {}).get("updated_at")
        or fecha_reunion
    )
    add_item(_fecha_historial(validation_at), current_validation_event)
    if row.get("_agenda_stage") == "reagendar":
        add_item(
            _fecha_historial(row.get("_seg", {}).get("updated_at") or fecha_reunion),
            "Motivo de reagenda",
...
def _bant_evaluation(bant):
    ...
    return (
        f'<div style="border:1px solid #d9dfda;background:#fff;border-radius:10px;'
...
def render_detalle(row):
    ...
    st.markdown(
        ...
        f'border-radius:9px">{_safe_html(LABEL_ETAPA_AGENDA[row["_agenda_stage"]])}</span>'
    ...
    if st.button(
        "Ver ICP acordado →",
        key=f"bambutech_icp_link_{reunion_id}",
        help="Abrir el resumen ICP confirmado en Onboarding",
    ):
        st.session_state["bambutech_scroll_to_icp"] = True
        st.switch_page("pages/17_BambuTech_Onboarding.py")
    if not is_quote:
        st.markdown(_bant_evaluation(bant), unsafe_allow_html=True)
    if not is_quote:
        st.markdown(
            '<div style="background:#f4f6f4;border:1px solid #d9dfda;border-radius:8px;'
            'padding:9px 12px;margin-top:9px;font-size:11px;color:#208d25;line-height:1.45">'
    ...
    if is_quote:
        st.info(
            "Cotización válida automáticamente por interés inmediato y traspaso al equipo comercial de BambuTech Services."
        )
    elif future:
...
def run():
    ...
    render_client_nav("18_BambuTech_Validacion", "bambutech")
    st.markdown(
        f"""
        <style>
        button[kind="primary"] {{
        ...
        div[class*="st-key-bambutech_icp_link_"] button {{
        ...
        div[class*="st-key-bambutech_icp_link_"] button p {{
            color:{GBS_PURPLE}!important;
        ...
        div[data-testid="stLayoutWrapper"]:has(> .st-key-bambutech_meeting_table_header) {{
            position:sticky!important;
            top:2.75rem!important;
            z-index:99999!important;
            background:linear-gradient(90deg,#171918,#38d430)!important;
            padding:9px 12px!important;
            border-radius:10px!important;
            box-shadow:0 4px 10px rgba(15,23,42,.10)!important;
            width:100%!important;
        }}
        .st-key-bambutech_meeting_table_header {{
            background:transparent!important;
        }}
        .st-key-bambutech_meeting_table_header button {{
            background:#fff!important;
            min-height:36px!important;
        }}
        div[data-testid="stVerticalBlock"]:has(.st-key-bambutech_meeting_table_header),
        div[data-testid="stVerticalBlockBorderWrapper"]:has(.st-key-bambutech_meeting_table_header) {{
    ...
    selected_range = st.session_state.get("bambutech_date_range", (month_start, month_end))
    ...
    query = st.session_state.get("bambutech_search", "")
    selected_agenda = st.session_state.get("bambutech_agenda_stage", "all")
    if selected_agenda not in {"all", *ETAPAS_AGENDA}:
        selected_agenda = "all"
        st.session_state["bambutech_agenda_stage"] = "all"
    selected_validation = st.session_state.get(
        "bambutech_validation_status",
        "all",
    )
    if selected_validation not in {"all", *ESTATUS_VALIDACION}:
        selected_validation = "all"
        st.session_state["bambutech_validation_status"] = "all"
    ...
    if not enriched.empty and selected_agenda != "all":
        enriched = enriched[
            enriched["_agenda_stage"] == selected_agenda
        ].copy()
    ...
    with filter_cols[3]:
        st.selectbox(
            ...
            label_visibility="visible",
            key="bambutech_validation_status",
        )
    with filter_cols[4]:
        if st.button("Actualizar", use_container_width=True, key="bambutech_refresh"):
            st.cache_data.clear()
            st.session_state["bambutech_kpi_filter"] = None
            st.rerun()
