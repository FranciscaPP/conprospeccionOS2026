...
def _status_label(row, seg):
    ...
    if "realiz" in raw or "completed" in raw or "vÃ¡lid" in raw or "valid" in raw:
...
function cpTab(m){return `<div class="grid"><div class="field2"><label>Evaluacion CP</label><select onchange="setField(${m.id},'cp',this.value)">${opt(cps,m.cp)}</select></div><div class="field2"><label>ICP</label><select onchange="setField(${m.id},'icp',this.value)">${opt(["No evaluado","Cumple","No cumple"],m.icp)}</select></div><div class="wide"><span class="block-title">BANT</span><div class="evidence">${Object.keys(m.bant).map(k=>`<label class="evi"><input type="checkbox" ${m.bant[k]?"checked":""} onchange="const m=current();const old={...m.bant};m.bant['${k}']=this.checked;addHistory(m,'BANT',old,{...m.bant});persist();render()"> ${k}<small>${m.bant[k]?"Si":"No"}</small></label>`).join("")}</div></div>${field("just","Justificacion visible al cliente","textarea")}${field("notes","Nota interna Conprospeccion","textarea")}</div><span class="block-title" style="margin-top:12px">Evidencias</span><div class="evidence">${m.evidence.map(e=>`<div class="evi">${e.valid?"âœ“ ":""}${esc(e.type)}<small>${esc(e.name)}</small></div>`).join("")||""}<button class="ghost" onclick="addEvidence('Archivo')">Subir archivo</button><button class="ghost" onclick="addEvidence('Enlace')">Pegar enlace</button><button class="ghost" onclick="addEvidence('Comentario')">Agregar comentario</button></div><div class="btn-row"><button class="primary" onclick="saveSection('Evaluacion CP')">Guardar Evaluacion CP</button></div>`}
