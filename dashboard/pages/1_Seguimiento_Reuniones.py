...
meetings=meetings.map(m=>({...m,caseStatus:m.caseStatus||"Abierto",clientVal:m.status==="Reunión cancelada"?"No necesaria":m.clientVal,clientActor:m.clientActor||m.contact||"",clientTimeline:m.clientTimeline||buildClientTimeline(m),historyVisibility:m.historyVisibility||{}}));
...
function setField(id,field,value){const m=meetings.find(x=>x.id===id);const old=m[field];m[field]=value;if(field==="status"&&value==="Reunión cancelada"){const oldClient=m.clientVal;m.clientVal="No necesaria";m.clientTimeline=buildClientTimeline(m);addHistory(m,"Evaluación Cliente",oldClient,"No necesaria")}if(field==="status"&&old==="Reunión cancelada"&&value!=="Reunión cancelada"&&m.clientVal==="No necesaria"){m.clientVal="Pendiente";m.clientTimeline=buildClientTimeline(m)}addHistory(m,labelField(field),old,value);if(["clientVal","clientReason","clientComment","clientEvidence","cpResponse"].includes(field)){recordClientEvent(m,field,old,value)}selected=id;panelOpen=true;if(field==="cp"&&value&&value!=="Pendiente")tab="Evaluación CP";if(field==="status"&&["Reunión cancelada","Reagendar reunión"].includes(value))tab="Información";if(field==="final")tab="Estado Final";if(field==="clientVal")tab="Evaluación Cliente";persist();saveMeeting(m,labelField(field),true);render()}
