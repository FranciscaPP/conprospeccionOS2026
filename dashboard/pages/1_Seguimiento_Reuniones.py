...
function visibleHistory(m){const allowed=["Fecha Agenda","Fecha Reunión realizada","Etapa Agenda","Evaluación CP","Evaluación Cliente"];const base=[];if(m.scheduledDate)base.push({when:m.scheduledDate,user:"Sistema",field:"Fecha Agenda",description:m.scheduledDate,manual:false,visibility:"Solo uso interno"});base.push({when:m.date||"Sin fecha",user:"Sistema",field:"Fecha Reunión realizada",description:`${m.date||""} ${m.time||""}`,manual:false,visibility:"Solo uso interno"});const filtered=(m.history||[]).map((h,idx)=>({...h,_idx:idx,user:String(h.user||"").toUpperCase()==="GHL"?"Sistema":h.user})).filter(h=>h.manual||allowed.includes(h.field));return base.concat(filtered)}
