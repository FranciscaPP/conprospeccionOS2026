...
function clientIcpStatus(meeting: Meeting) {
  return meeting.meetingStatus === "cancelled" ? false : true;
}

function hasClientValue(value?: string) {
...
export function MeetingDrawer({ meeting, open, onClose, mode }: MeetingDrawerProps) {
  ...
  const currentFlowStatus = getMeetingFlowStatus(meeting);
    ...
    try {
      const response = await fetch("/api/internal/meetings", {
        ...
        body: JSON.stringify({
          ...
          commercialStatus: updates.commercialStatus,
          nextStep: updates.nextStep,
          preparationInfo: updates.preparationInfo ?? "",
          cpBANT: updates.cpBANT ?? [],
        }),
      });
      ...
      setSaveError(error instanceof Error ? error.message : "No se pudo guardar la reunión.");
      await refreshMeetings();
    }
  };
  ...
  if (isClientMode) {
    ...
    const flowStatus = currentFlowStatus;
    ...
    timeline.push({
      title: meetingFlowStatusLabels[flowStatus],
      detail: "Estado actual",
      tone:
        flowStatus === "evaluation_closed_valid"
          ? "ok"
          : flowStatus === "evaluation_closed_not_valid" || flowStatus === "cancelled_meeting"
            ? "bad"
            : flowStatus === "client_requests_review"
              ? "warn"
              : "info",
    });
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <StatusBadge status={flowStatus} label={meetingFlowStatusLabels[flowStatus]} size="sm" />
            ...
            <div className="space-y-5 px-5 py-5">
              <section className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                  {hasClientValue(meeting.company) && <ClientField label="Empresa" value={meeting.company} />}
                  {hasClientValue(meeting.country) && <ClientField label="País" value={meeting.country} />}
                  {hasClientValue(displayContactName(meeting)) && <ClientField label="Contacto" value={displayContactName(meeting)} />}
                  {hasClientValue(meeting.companyWebsite) && <ClientField
                    label="Website"
                    value={
                  ...
                  />}
                  {hasClientValue(meeting.jobTitle) && <ClientField label="Cargo" value={meeting.jobTitle} />}
                  {hasClientValue(meeting.companyLinkedinUrl) && <ClientField
                    label="LinkedIn empresa"
                    value={<ClientExternalLink href={meeting.companyLinkedinUrl}>Ver empresa</ClientExternalLink>}
                  />}
                  {hasClientValue(meeting.leadEmail) && <ClientField label="Email" value={meeting.leadEmail} />}
                  {hasClientValue(meeting.contactLinkedinUrl) && <ClientField
                    label="LinkedIn contacto"
                    value={<ClientExternalLink href={meeting.contactLinkedinUrl}>Ver perfil</ClientExternalLink>}
                  />}
                  {hasClientValue(meeting.leadPhone) && <ClientField label="Teléfono" value={meeting.leadPhone} />}
                  {hasClientValue(meeting.meetingUrl) && <ClientField
                    label="Link reunión"
              ...
              {hasClientValue(meeting.preparationInfo) && (
                <section className="space-y-2">
                  <ClientSectionTitle icon={<MessageSquare className="h-4 w-4" />} number={2} title="Información para reunión" />
                  <p className="line-clamp-6 text-sm leading-6 text-muted-foreground">
                    {meeting.preparationInfo}
                  </p>
                </section>
              )}
              ...
              <section className="space-y-2">
                ...
                {futureMeeting ? (
                  <StatusBadge status="waiting_validation" label="Reunión futura" size="sm" />
                ) : (
              ...
              {!futureMeeting && <Separator />}

              {!futureMeeting && <section className="space-y-3">
        ...
        <SheetHeader className="border-b border-border p-4 pb-3 sm:p-[18px]">
          <div className="flex items-start justify-between gap-4">
            ...
            <StatusBadge
              ...
              label={meetingFlowStatusLabels[currentFlowStatus]}
            />
          </div>
        </SheetHeader>
                  ...
                  <div className="space-y-2">
                    {!hasClientValue(meeting.preparationInfo) && (
                      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                        Falta Información para reunión. La SDR debe completarla manualmente.
                      </div>
                    )}
                    <Label className="text-xs text-muted-foreground">Información para reunión</Label>
                    <Textarea
                      value={formData.preparationInfo || ""}
                      onChange={(event) => setFormData({ ...formData, preparationInfo: event.target.value })}
                      placeholder="Agregar información de preparación para la reunión"
                      rows={4}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    {meeting.cpBANT.length === 0 && (
                      <p className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
                        Falta Validación SDR BANT. La SDR debe completarla manualmente.
                      </p>
                    )}
                    <BantSelector
                ...
                <section className="space-y-3">
                  ...
                  <ol className="relative space-y-3 border-l border-border pl-5">
                    <li className="relative">
                      <span className="absolute -left-[23px] top-1 h-2.5 w-2.5 rounded-full border-2 border-white bg-[#1d4ed8]" />
                      <p className="font-display tnum text-[11px] text-muted-foreground">
                        {safeFormatDate(meeting.meetingDate, "d MMM yyyy · HH:mm")}
                      </p>
                      <p className="text-sm font-semibold text-foreground">Reunión agendada</p>
                    </li>
                    {meeting.meetingStatus === "completed" && (
                      <li className="relative">
                        <span className="absolute -left-[23px] top-1 h-2.5 w-2.5 rounded-full border-2 border-white bg-emerald-500" />
                        <p className="text-sm font-semibold text-foreground">Reunión realizada</p>
                      </li>
                    )}
                    {meeting.cpValidation !== "waiting_validation" && (
                      <li className="relative">
                        <span className="absolute -left-[23px] top-1 h-2.5 w-2.5 rounded-full border-2 border-white bg-amber-500" />
                        <p className="text-sm font-semibold text-foreground">Evaluación CP registrada</p>
                        <p className="text-xs text-muted-foreground">{cpValidationLabels[meeting.cpValidation]}</p>
                      </li>
                    )}
                    <li className="relative">
                      <span className="absolute -left-[23px] top-1 h-2.5 w-2.5 rounded-full border-2 border-white bg-[#333]" />
                      <p className="text-[11px] font-medium uppercase tracking-wide text-muted-foreground">Estado actual</p>
                      <StatusBadge
                        status={currentFlowStatus}
                        label={meetingFlowStatusLabels[currentFlowStatus]}
                        size="sm"
