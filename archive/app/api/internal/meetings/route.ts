...
import type { BANTCriteria } from "@/lib/types";
...
function operationalDataPatch(
  currentRawData: Record<string, unknown> | null | undefined,
  preparationInfo: string | undefined,
  cpBANT: BANTCriteria[] | undefined
) {
  if (preparationInfo === undefined && cpBANT === undefined) return {};

  const rawData = { ...(currentRawData ?? {}) };
  if (preparationInfo !== undefined) {
    rawData.meetingInfo = preparationInfo.trim();
    rawData.preparationInfo = preparationInfo.trim();
    rawData["informacin_de_preparacin_para_la_reunin"] = preparationInfo.trim();
  }
  if (cpBANT !== undefined) {
    const value = cpBANT.join(",");
    rawData.cpBANT = value;
    rawData.validationSdrBant = value;
    rawData["validacin_sdr_bant"] = value;
  }
...
export async function PATCH(request: Request) {
  try {
    ...
    let currentRawData: Record<string, unknown> | null | undefined;
    if (body.preparationInfo !== undefined || body.cpBANT !== undefined) {
      let currentRows = await supabaseGet<SupabaseMeetingRow>(
        "reuniones",
        { select: "id,ghl_appointment_id,raw_data", ghl_appointment_id: `eq.${body.id}`, limit: "1" },
        config
      );
      if (currentRows.length === 0 && /^\d+$/.test(body.id)) {
        currentRows = await supabaseGet<SupabaseMeetingRow>(
          "reuniones",
          { select: "id,ghl_appointment_id,raw_data", id: `eq.${body.id}`, limit: "1" },
          config
        );
      }
      currentRawData = currentRows[0]?.raw_data;
    }
    ...
    const payload = Object.fromEntries(
      Object.entries({
        ...
        ...operationalDataPatch(currentRawData, body.preparationInfo, body.cpBANT),
