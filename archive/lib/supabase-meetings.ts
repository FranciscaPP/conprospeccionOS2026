...
function firstRawText(row: SupabaseMeetingRow, ...keys: string[]) {
  for (const key of keys) {
    const value = rawText(row, key);
    if (value) return value;
  }
  return "";
}

function parseBANT(value: unknown): BANTCriteria[] {
  const normalized = clean(value).toLowerCase();
  if (!normalized) return [];

  const detected = new Set<BANTCriteria>();
  const aliases: Record<string, BANTCriteria> = {
    b: "budget",
    budget: "budget",
    presupuesto: "budget",
    a: "authority",
  ...
  normalized
    ...
    .filter(Boolean)
    .forEach((token) => {
      const criteria = aliases[token];
      if (criteria) detected.add(criteria);
    });
  return [...detected];
}
...
export function mapSupabaseRowsToMeetings(
  ...
  const meetings = cleanedRows.rows.map((row) => {
    ...
    const meetingStatus = mapMeetingStatus(row.estado_reunion);
    const preparationInfo = firstRawText(
      row,
      "meetingInfo",
      "preparationInfo",
    ...
    const cpBANT = parseBANT(
      firstRawText(
        row,
        "validacin_sdr_bant",
        "contact.validacin_sdr_bant",
        "validationSdrBant",
        "cpBANT"
      )
    );
    ...
    return {
      ...
      cpBANT: cpBANT.length ? cpBANT : validation.cpBANT,
      ...
      preparationInfo: preparationInfo || undefined,
