...
select
  ...
  (select count(*) from public.snov_campaigns where lower(status) in ('active', 'paused')) as campanas_snov_monitoreadas,
...
select
  ...
  case
    ...
    else round(coalesce(correos.respuestas_semana, 0)::numeric / nullif(correos.emails_enviados_semana, 0), 4)
  end as conversion_email_respuesta_semana,
  case
    when coalesce(llamadas.llamadas_dia, 0) = 0 then 0
    else round(coalesce(reuniones.reuniones_dia, 0)::numeric / nullif(llamadas.llamadas_dia, 0), 4)
...
select
  ...
  case
    ...
    else round(coalesce(r.reuniones_dia, 0)::numeric / nullif(l.llamadas_dia, 0), 4)
  end as conversion_llamadas_reuniones_dia,
  case
    when coalesce(r.reuniones_dia, 0) = 0 then 0
    else round(coalesce(r.reuniones_validas_dia, 0)::numeric / nullif(r.reuniones_dia, 0), 4)
