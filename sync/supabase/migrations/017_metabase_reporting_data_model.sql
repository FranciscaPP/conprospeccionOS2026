-- 017 - Modelo limpio para Metabase y backfill seguro de relaciones.
-- No borra tablas ni columnas. Completa solo campos NULL usando relaciones confiables.

create table if not exists public.backup_reuniones_pre_017 as
select * from public.reuniones;

create table if not exists public.backup_llamadas_pre_017 as
select * from public.llamadas;

create table if not exists public.backup_pagos_sdr_pre_017 as
...
select
  ...
  case when coalesce(l.total_llamadas, 0) = 0 then 0 else round(r.reuniones_agendadas::numeric / nullif(l.total_llamadas, 0), 4) end as tasa_llamada_a_reunion,
  case when coalesce(r.reuniones_agendadas, 0) = 0 then 0 else round(r.reuniones_validas::numeric / nullif(r.reuniones_agendadas, 0), 4) end as tasa_reunion_valida,
...
select
  ...
  case
    when extract(day from d.fecha) = 0 then 0
    else round(coalesce(rm.reuniones_validas_mes, 0)::numeric / extract(day from d.fecha) * extract(day from (date_trunc('month', d.fecha) + interval '1 month - 1 day')), 2)
  end as proyeccion_cierre_mes
from dim d
join public.clientes c on c.slug = d.cliente_slug
left join public.cliente_metas cm on cm.cliente_slug = c.slug and cm.periodo = 'contrato'
left join (
...
select current_date as fecha_alerta, 'reunion_sin_cliente_id' as tipo_alerta, 'alta' as severidad,
  r.cliente_id, cl.nombre as nombre_cliente, r.sdr_id, s.nombre as nombre_sdr,
...
select current_date, 'reunion_sin_sdr_id', 'alta',
  r.cliente_id, cl.nombre, r.sdr_id, s.nombre,
  'reuniones', r.id::text,
...
select current_date, 'llamada_sin_sdr_id', 'alta',
  l.cliente_id, cl.nombre, l.sdr_id, s.nombre,
  'llamadas', l.id::text,
...
select current_date, 'cliente_sin_meta_definida', 'media',
  c.id, c.nombre, null::uuid, null::text,
  'clientes', c.id::text,
...
select current_date, 'sdr_sin_cliente_asignado', 'media',
  null::uuid, null::text, s.id, s.nombre,
  'sdrs', s.id::text,
